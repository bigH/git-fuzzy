...
Type to filter. '"${WHITE}Enter${NORMAL} to ${GREEN}ACCEPT${NORMAL}"'
  ...
  '"${YELLOW}${BOLD}∆${NORMAL} ${GREEN}working copy${NORMAL}  ${WHITE}$GIT_FUZZY_BRANCH_WORKING_COPY_KEY${NORMAL}        $BRANCH_HEADER_BRANCH_CHECKOUT"'
    '"${YELLOW}${BOLD}∆${NORMAL} ${GREEN}merge-base${NORMAL}  ${WHITE}$GIT_FUZZY_BRANCH_MERGE_BASE_KEY${NORMAL}         ${GREEN}${BOLD}checkout ${YELLOW}${BOLD}📁${NORMAL} ${WHITE}$GIT_FUZZY_BRANCH_CHECKOUT_FILE_KEY${NORMAL}"'
...
'
  ...
  gf_fzf_one -m \
             ...
             $BRANCH_CHECKOUT_BINDING \
             --bind $GIT_FUZZY_BRANCH_CHECKOUT_FILE_KEY':execute(git fuzzy helper branch_checkout_files {1})' \
