...
gf_log_warning() {
  printf "%d [%s%sWRN%s]" "$$" "$YELLOW" "$BOLD" "$NORMAL" >> "$GF_LOG_LOCATION"
  printf " %s" "$@" >> "$GF_LOG_LOCATION"
  echo >> "$GF_LOG_LOCATION"
}

gf_log_error() {
  printf "%d [%s%sERR%s]" "$$" "$RED" "$BOLD" "$NORMAL" >> "$GF_LOG_LOCATION"
  printf " %s" "$@" >> "$GF_LOG_LOCATION"
  echo >> "$GF_LOG_LOCATION"
}
...
gf_log_debug() {
  if [ -n "$GF_DEBUG_MODE" ]; then
    printf "%d [%s%sDBG%s]" "$$" "$GRAY" "$BOLD" "$NORMAL" >> "$GF_LOG_LOCATION"
    printf " %s" "$@" >> "$GF_LOG_LOCATION"
    echo >> "$GF_LOG_LOCATION"
  fi
}
...
gf_log_command() {
  if [ -n "$GF_COMMAND_DEBUG_MODE" ]; then
    if [ "$#" -gt 0 ]; then
      printf "%d [%s%sCMD%s]" "$$" "$GREEN" "$BOLD" "$NORMAL" >> "$GF_LOG_LOCATION"
...
gf_log_internal() {
  if [ -n "$GF_INTERNAL_COMMAND_DEBUG_MODE" ]; then
    if [ "$#" -gt 0 ]; then
      printf "%d [%s%sCMD%s] (internal)" "$$" "$GRAY" "$BOLD" "$NORMAL" >> "$GF_LOG_LOCATION"
